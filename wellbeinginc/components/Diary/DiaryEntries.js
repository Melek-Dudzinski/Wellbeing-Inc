...
function DiaryEntries ({user}) {
    ...
    return(
        ...
        {entries && entries.map((entry,id) => (
            <div key={id}>
                <DiaryEntry date={entry.date} exerciseData = {entry.exerciseSection} dietData = {entry.dietSection} stepsData={entry.stepsSection} sleepData={entry.sleepSection} user={user} canEditStyle={turnEdit}></DiaryEntry>
            </div>
        ))}
        </>
    );
